...
export class Tabs extends Component<PropsType, StateType> {
  ...
  renderContent(getSubElements = this.getSubElements()) {
    ...
    return <View className={contentCls} style={contentStyle} ref={this.setContentLayout}>
      {
        tabs.map((tab, index) => {
  ...
  render() {
    ...
    const content = [
      <View key="tabBar" className={`${prefixCls}-tab-bar-wrap`}>
    ...
    return <View className={`${prefixCls} ${prefixCls}-${tabDirection} ${prefixCls}-${tabBarPosition}`}>
      {
