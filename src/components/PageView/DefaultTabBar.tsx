...
import Gesture, { IGestureStatus } from './gesture';
...
export class DefaultTabBar extends React.PureComponent<PropsType, StateType> {
  ...
  render() {
    ...
    return <View className={`${cls} ${prefixCls}-${tabBarPosition}`} style={style}>
      {showPrev && <View className={`${prefixCls}-prevpage`}></View>}
      <Gesture {...onPan }
        direction={isTabBarVertical ? 'vertical' : 'horizontal'}
      >
        <View role="tablist" className={`${prefixCls}-content`} style={transformStyle} ref={this.setContentLayout}>
          {Tabs}
          {
            renderUnderline ? renderUnderline(underlineProps) :
              <View {...underlineProps}></View>
          }
        </View>
      </Gesture>
      {showNext && <View className={`${prefixCls}-nextpage`}></View>}
    </View>;
  }
}
